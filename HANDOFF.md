...
- `src/git.ts` — `node:child_process` `execFile` helpers (no shell interpolation), Result types, `createWorktree`/`removeWorktree`/`getProjectId`/sibling path resolution. Runs in both the Bun runtime and node tests.
- `src/terminal.ts` — terminal launch (tmux/macOS/Linux/Windows), self-deleting temp scripts, escaping. `node:child_process` based.
...
- `test/worktree.integration.test.ts` — 6 real-git tests (create/remove worktree, project-ID sharing, existing-branch checkout, open→delete→idle lifecycle), green
...
4. **Worktree — DONE** — `openWorktree` creates the worktree as a sibling of the repo, forks the session into it (`session.fork` with `query.directory`), launches a terminal via `worktree.launchCommand --session <forkedId>`, and records the mapping in `src/state.ts`. Cleanup mirrors opencode-worktree's `session.idle` + pending-delete: `tlc_worktree_delete` marks pending, `handleSessionIdle` commits + removes the worktree when the forked session ends. Git/terminal/state layers are node-portable and the full lifecycle is integration-tested against a real temp repo. **Live-validated** (2026-08-12): plugin loaded into a scratch repo, `tlc_worktree` created the sibling worktree + forked the session + opened Terminal.app running the forked session, then `tlc_worktree_delete` + session.idle removed the worktree and cleared state.
